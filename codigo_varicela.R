...
if(FALSE){
  vacine_indic_delay=rep(NA,T_final)
  vacine_indic_delay[true_indice_inter:T_final]=1
  vacine_indic=rep(NA,T_final)
  vacine_indic[indice_inter:T_final]=1
  covid_indic=rep(NA,T_final)
  covid_indic[146:T_final]=1

  dados=rbind(dados,vacine_indic,vacine_indic_delay,covid_indic)

  row.names(dados)=c('00 a 04 anos',
                     '05 a 09 anos',
                     '10 a 14 anos',
                     '15 a 49 anos',
                     '50 anos e mais',
                     'Indicadora de vacina',
                     'Indicadora de vacina (com delay)',
                     'Indicadora de pandemia')

  write.csv(dados,'varicela_internacoes_completo.csv')

  names(pre_exp)=names(dados)
  row.names(pre_exp)=c('00 a 04 anos',
                     '05 a 09 anos',
                     '10 a 14 anos',
                     '15 a 49 anos',
                     '50 anos e mais')
  write.csv(pre_exp,'varicela_exposicao_completo.csv')
}
...
show_fit(resultado,smooth=FALSE,t_offset=1)
