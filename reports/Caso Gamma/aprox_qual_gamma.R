...
# conj_prior=update_FGamma(conj_prior,1)
...
exp(ft[1,]+Qt[1,1]/2)
exp(ft[1,]-ft[2,]+(Qt[1,1]+Qt[2,2]-2*Qt[1,2])/2)
-(ft[2,]+Qt[1,2])*exp(ft[1,]+Qt[1,1]/2)
exp(ft[1,]+Qt[1,1]/2)+0.5*ft[1,]-exp(-ft[1,]+Qt[1,1]/2)/12-11/12
...
f=function(x){(-digamma(n*x+1)+log(tau*x))*f_densi(x)}
integrate(f,0,Inf)$value

##################################################

conj_post=convert_FGamma_Normal(ft,Qt)
...
f=function(x){dgamma(x,a,b)}
c=integrate(f,0,Inf)$value
plot(x,f(x)/c,lty=1,type='l',ylim=c(0,1))
# lines(x,dlnorm(x,ft[1,],sqrt(Qt[1,1])),lty=2)

conj_post=conj_prior#update_FGamma(conj_prior,1)
conj_norm=convert_Normal_FGamma(conj_post)
ft=conj_norm$ft
Qt=conj_norm$Qt
...
# lines(x,dgamma(x,(conj_prior$n+3)/2,conj_prior$n*log(conj_prior$tau/conj_prior$n)-conj_prior$theta),lty=2)
# lines(x,dgamma(x,(conj_prior$n+2)/2,conj_prior$n*log(conj_prior$tau/conj_prior$n)-conj_prior$theta),lty=2)
n=conj_post$n
tau=conj_post$tau
theta=conj_post$theta
print(conj_post)

f=function(x){exp(lgamma(n*x+1)-n*lgamma(x)+(theta-n*log(tau))*x)}
c=integrate(f,0,Inf)$value
lines(x,f(x)/c,lty=3,col='red')
