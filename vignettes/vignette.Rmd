...
3. In the following section we discuss how the user can specify the observational model;
4. With the model structure defined, we can proceed presenting the tools for fitting a defined model, also showing the auxiliary functions that help the user to analyse the fitted model;
...
Let us assume that the user is interested in analyzing a Time Series $Y_t$, $t=1,...T$, which can be described by the following model:
...
Notice that we **did not** make any assumptions on the dimension of $Y_t$ or $\eta_t$, indeed, this package offers tools for the fitting time series of vectors (as in the Multinomial case) ad/or with multiple unknown parameters (as in the Normal with unknown variance case).

Naturally, the manual specification of all the structural components of the model can be quite tiresome. With that in mind, our package offers a wide and ever expanding set of auxiliary functions that aim to help the user to specify the struture of a model. In the next section we will explore those tools.
...
$$
\begin{align}
\theta_{it} &= \theta_{it-1}+\theta_{i+1\ t-1}+\epsilon_{it}, i=1,...,k-1\\
...
Notice that the default values of each argment leads to a first order polynomial block with no temporal dynamic, besides, the default prior variance of the first parameter is not specified. Indeed, if the user does not specify the prior, we assume that $\theta_{1i} \sim \mathcal{N}(0,1), i=2,...,k$ and $\theta_{11}/Var(X_t) \sim \mathcal{N}(0,1)$, where $X_t$ represents the values of $F_t$ associated with $\theta_{11}$ (i.e., the ones passed through the `...` argumente). If $Var(X_t)=0$ or $X_t$ only assumes $0$ and $1$ values, then we assume that $\theta_{11} \sim \mathcal{N}(0,1)$.
...
- The arguments `D` and `W` define the variance of fluctuations of $\phi_t$ through time;
- The arguments `m0_state`, `C0_state` are the parameter for the prior for the latent states $\theta_1, ...\theta_{1-k}$;
- The arguments `noise_var` and `D` define the variance of fluctuations of $\theta_t$ through time;
...
$$
\begin{align}
Y_t|\mu_t,\Sigma &\sim \mathcal{N}_k\left(\mu_t,\Sigma\right),\\
...
Intuitively, the `mu` argument must be a character vector of size $k$ containing the names of the linear predictors associated with each $\mu_{i.}$. The user must also specify one (and only one) of `Tau`, `Sigma` or `Sd`. If the user provides `Sigma`, $\Sigma$ is assumed to be the passed value; if the user provides `Tau`, $\Sigma$ is assumed to be the inverse of the passed matrix (i.e., `Tau` is the precision matrix); if the user provides `Sd`, $\Sigma$ is assumed to be such that the standard deviation of the observations is equal to diagonal of `Sd` and the correlation between observations is assumed the be equal to the off-diagonal elements of `Sd`. 
...
Just as before, the `mu` argument must be a character representing the name of the linear predictor associated with $\mu_t$. The user must also specify one (and only one) of `Tau`, `Sigma` or `Sd`, which must a character representing the name of the linear predictor associated with $\tau_t$,$\tau_t^{-1}$ or $\tau_t^{-1/2}$, respectively.
...
$$
\begin{align}
Y_t|\mu_{t},\Sigma_t &\sim \mathcal{N}_2\left(\mu_t,\Sigma_t\right),\\
\mu_t&=\begin{bmatrix}\mu_{1t}\\ \mu_{1t}\end{bmatrix},\\
\Sigma_t&=\begin{bmatrix}\tau_{1t}^{-1} & \sqrt{\tau_{1t}\tau_{2t}}\rho\\ \sqrt{\tau_{1t}\tau_{2t}}\rho & \tau_2^{-1}\end{bmatrix},\\
...
mu2 <- cumsum(rnorm(T, 0, 0.2)) - 1
Sigma <- matrix(c(1, 1, 1, 4), 2, 2)
data <- matrix(rnorm(T * 2), T, 2) %*% chol(Sigma) + cbind(mu1, mu2)
var(data)
```
...
outcome <- Normal(
  mu = c("mu_1", "mu_2"),
  Tau = matrix(c("sigma_1", "rho", "rho", "sigma_2"), 2, 2),
  outcome = data
)
...
As seen previously, to fit models one must use the function `fit_model`. In this section we detail the usage of this function, while also highlighing the usage of each argumente. We also present tools for sensibility analysis and graphical analysis of fitted models.
...
The `outcomes` argument must be a list of `dlm_distr` objects created as described in the previous section. `pred_cred` is a numeric value representing the desired credibility of the predictions made while fitting (if `pred_cred` is not a valid value, no predictions are made). `smooth` is a boolean indecating if the smoothed distribuition of the latent variables should be calculated (generally we recommend the users to not change this value, as the computational cost of smoothing is really low). `p_monit` and `c_monit` control the sensibility of the automated monitoring and we shall discuss their usage in a later section. Lastly, all other arguments passed are assumed to be `dlm_block` objects, created as in section 2, defining the model structure.

Bellow we present an example of usage of the `fit_model` function:
...
Notice that the coefficients shown are those of the last observation (time 200), but one can also see the summary of any paticular time by changing the argument `t`:

```{r}
summary(fitted_data, t = 100)
```

For more details about the usage of the `report_dlm` function, see the associated documentation (`help(report_dlm)`).

Naturally, the user might want to acess the estimated values of each parameter, along with other informations about the fitted model. The `fitted_dlm` class (which is the output of the `fit_model` function) provides the following attributes:

- `mt`: A $n \times T$ matrix containg the filtered mean of the latent variables for each time;
- `mts` (if `smooth` is `TRUE`): A $n \times T$ matrix containg the smoothed mean of the latent variables for each time;
- `Ct`: A 3D-array containing the filtered covariance matrix of the latent variable for each time. Dimensions are $n \times n \times T$;
- `Cts` (if `smooth` is `TRUE`): A 3D-array containing the smoothed covariance matrix of the latent variable for each time. Dimensions are $n \times n \times T$;
- `at`: A $n \times T$ matrix containg the one-step-ahead mean of the latent variables at each time;
- `Rt`: A 3D-array containing the one-step-ahead covariance matrix for latent variables at each time. Dimensions are $n \times n \times T$;
- `ft`: A $k \times T$ matrix containg The one-step-ahead linear predictor for each time;
- `Qt`: A 3D-array containing the one-step-ahead covariance matrix for the linear predictor for each time. Dimensions are $k \times k \times T$;
- `FF`: A 3D-array containing the regression used matrix at each time. It's dimension are $n \times k \times T$;
- `G`: A 3D-array containing the state evolution matrix at each time. It's dimension are $n \times n \times T$;
- `G_labs`: A 3D-array containing the type of calculation associated with each elemente of G. It's dimension are the same as `G`;
- `D`: A 3D-array containing the discount factor matrix for each time. It's dimension are $n \times n \times T$;
- `W`: A 3D-array containing the covariance matrix of the noise for each time. It's dimension are $n \times n \times T$;
- `shock_var`: A 3D-array containing the effective covariance matrix of the noise for each time (taking into account both `W` and `D`). It's dimension should be the same as `W` and `D`;
- `outcome` List: The same as the argument outcome (same values);
- `pred_names` Vector: The names of the linear predictors.

Where $n$ is the number of latent parameters, $k$ is the number of linear predictor and $T$ is the sample size.

Most users do not have to worry about those values thou, since adequate functions are provided to extract the most frequently used information. 
