...
Another particularity of the Gamma outcome is that the user may set the shape parameter $\phi_t$ to a known constant. In that case, the user must specify the structure to the mean parameter $\mu_t$ (he is not allowed to specify neither $\beta_t$ nor $\sigma_t$). In general, it is not expected that the shape parameter is known, still, there are some important applications where it common the use of Exponential Model ($\phi_t=1$) or some other particular cases of the Gamma distribution.
...
In the examples of this section, we will always use the parameters $\phi_t$ (when applicable) and $\mu_t$, but code used can be trivially adapted to other parametrizations.
...
Note that the above relationship implies that:
...
devtools::load_all()
# Structure
volatility=AR_block(h=1,order=1,noise_var=W,AR_support='free')

# Outcome
outcome <- Gamma(phi = 0.5, mu='h', outcome = data)

# Fitting and ploting
fitted_data <- fit_model(volatility, outcomes = outcome)
...
# Structure
volatility=AR_block(h=1,order=1,noise_var=W,AR_support='free')
shape=polynomial_block(phi=1)

# Outcome
outcome <- Gamma(phi = 'phi', mu='h', outcome = data)

# Fitting and ploting
fitted_data <- fit_model(shape,volatility, outcomes = outcome)
plot(fitted_data,smooth=TRUE)
```
